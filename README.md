...
現在のMVPは、Pillowによるsoft mask抽出、AA edge fringe補修、binary inpaint mask内の透明穴fill、canvas alignment維持、draw order再合成、premultiplied alphaによるimport asset限定の品質検査、全体再合成motion preview、failed-part refinement planに加え、生成inpaintingのmock backendとoptional Diffusers / FLUX Fill adapter、候補ranking、レビュー後のqueue更新候補生成まで実装しています。意味segmentation、統合orchestrator、redraw、実PSD writerは未接続です。素材状態を変更する単一ソースは `asset_generation_queue.yaml` で、inpainting resultやselectionはcanonical queueを直接変更しません。manifest、mask manifest、layer mapもqueueから再生成し直接編集しません。`tools.psd_asset_builder` は空のPSDを作らず、実backend接続前は必ず `status: plan_only` を返します。
