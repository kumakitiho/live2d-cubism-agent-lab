...
canonical queue → 自動segmentation候補 → 人間レビュー → assignment plan → merge gate
...
現在のMVPは、review-firstの自動segmentation候補生成、Pillowによるsoft mask抽出、AA edge fringe補修、binary inpaint mask内の透明穴fill、canvas alignment維持、draw order再合成、premultiplied alphaによるimport asset限定の品質検査、全体再合成motion preview、failed-part refinement planに加え、生成inpaintingのmock backendとoptional Diffusers / FLUX Fill adapter、候補ranking、レビュー後のqueue更新候補生成まで実装しています。統合orchestrator、redraw、実PSD writerは未接続です。素材状態を変更する単一ソースは `asset_generation_queue.yaml` で、segmentation result、assignment、inpainting result、selection、manifest、mask manifest、layer mapはいずれも派生成果物としてcanonical queueを直接変更しません。`tools.psd_asset_builder` は空のPSDを作らず、実backend接続前は必ず `status: plan_only` を返します。
...
- 生成inpaintingはmockとoptional Diffusers / FLUX Fill adapter、局所合成、候補評価・rankingまで実装済みです。意味segmentation、統合orchestrator、redraw、Photoshop Plugin、実PSD writerは未接続です。
...
1. segmentation / redraw / 外部画像生成tool adapterと生成inpainting実modelのGPU fixture
