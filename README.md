...
MVPでは画像やPSDの実生成より、計画・queue・validationを優先します。素材状態を変更するのは `asset_generation_queue.yaml` だけです。manifestとlayer mapは `tools.asset_queue_builder` で再生成し、直接編集しません。`tools.psd_asset_builder` は空のPSDを作らず、実backend接続前は必ず `status: plan_only` を返します。
