...
失敗partだけrefinement queueへ戻す
  ↓ 生成inpainting・redraw backend（現在は未接続）
  ↓ queue builder
...
現在のMVPは、review-firstの自動segmentation候補生成、Pillowによるsoft mask抽出、AA edge fringe補修、binary inpaint mask内の透明穴fill、canvas alignment維持、draw order再合成、premultiplied alphaによるimport asset限定の品質検査、全体再合成motion preview、failed-part refinement planまで実装しています。生成inpainting、redraw、実PSD writerは未接続です。素材状態を変更する単一ソースは `asset_generation_queue.yaml` で、segmentation result、assignment、manifest、mask manifest、layer mapは派生成果物として直接queueを変更しません。`tools.psd_asset_builder` は空のPSDを作らず、実backend接続前は必ず `status: plan_only` を返します。
