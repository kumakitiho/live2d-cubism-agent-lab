...
現在のMVPは、Pillowによるsoft mask抽出、AA edge fringe補修、binary inpaint mask内の透明穴fill、canvas alignment維持、draw order再合成、premultiplied alphaによるimport asset限定の品質検査、全体再合成motion preview、failed-part refinement planまで実装しています。意味segmentation、生成inpainting、redraw、実PSD writerは未接続です。素材状態を変更する単一ソースは `asset_generation_queue.yaml` で、manifest、mask manifest、layer mapはqueueから再生成し直接編集しません。`tools.psd_asset_builder` は空のPSDを作らず、実backend接続前は必ず `status: plan_only` を返します。
...
`hidden_region_completer` のローカルbackendが実行できるのは `extract_and_edge_repair` と `transparency_fill` だけです。前者は `edge_extension_mask` のsource RGBAを追加してからtarget edge近傍のRGB fringeを補修し、後者はbinary `inpaint_mask` 内の完全透明穴だけを補完します。`inpaint` と `redraw` は未接続エラーで停止します。refinementはpreserve差分をsourceからの再抽出へ戻し、haloをedge repair、透明穴/coverage/allowed-change差分をtransparency fillの候補とします。preserve違反以外は現在方式より前へ戻らず、local方式を使い切った場合だけinpaint/redrawへ進めます。

mask manifestとasset quality reportは、このbreaking contract変更を明示するschema v2です。quality gateは、white halo、透明穴、明示extension coverage不足、protect領域の `preserve_region_difference_score`、edge/inpaint許可領域の差分、視覚再合成差分を別々に記録します。preserve領域の閾値は常に0です。count、allowed-change差分、`visual_reconstruction_difference_score` はCLIとquality YAMLのthresholdsで設定できます。quality、recomposer、motion、refinementは同じ `include_in_import: true` layer集合を使い、guide/mask assetを除外します。`overlap_margin_px` だけでtargetを自動膨張せず、desired coverageはtargetと `edge_extension_mask` だけから導出し、生成inpainting用の `inpaint_mask` と混同しません。refinementではinpaint/redrawのreview invariantと所有job operationsも同期します。

視覚差分はRGBAをpremultiplied alphaへ変換してから比較するため、完全透明ピクセルのRGBだけが違ってもfailureになりません。sourceに背景が含まれていてもモデル素材へ背景を含めない場合は、全import partのtarget/edge/inpaint領域とreconstruction alphaの和集合をforeground/reconstruction maskとし、その範囲だけをglobal評価します。reconstruction alphaを含めることで、宣言mask外へはみ出した不透明画素も検出します。各partにも同じ再合成差分を担当領域で計測し、global failureを可能な限りfailed partへ帰属させます。
