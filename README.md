...
`hidden_region_completer` のローカルbackendが実行できるのは `extract_and_edge_repair` と `transparency_fill` だけです。前者はAA edge近傍のRGB fringeだけを補修し、後者はbinary inpaint mask内の完全透明穴だけを補完します。`inpaint` と `redraw` は未接続エラーで停止します。refinementはhalo/source差分をedge repair、透明穴/coverage不足をtransparency fillの候補とし、現在方式より前へ戻らずlocal方式を使い切った場合だけinpaint/redrawへ進めます。

quality gateの現MVP閾値は、white halo、透明穴、明示extension coverage不足、protect mask内のsource画素差分、global再合成差分のすべてが0です。quality、recomposer、motion、refinementは同じ `include_in_import: true` layer集合を使い、guide/mask assetを除外します。`overlap_margin_px` だけでtargetを自動膨張せず、desired coverageはtargetとinpaint/edge-extension maskから導出します。refinementではinpaint/redrawのreview invariantと所有job operationsも同期します。
