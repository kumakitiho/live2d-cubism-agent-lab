...
inpaint候補はprotect領域をpremultiplied差分0で保持し、target/protectのうちedge extension/inpaintに含まれない領域を変更しない。宣言したtarget/protect/edge-extension/inpaintの外へ不透明画素を漏らさず、target必須coverageのalpha hole、実際に生成されたinpaint画素が既存target/protect/edge-extensionへ接するseamのalpha連続性と境界色差をgateする。`inpaint_mask` は変更許可領域であり全域opaqueを要求しない。standalone隠れpartで既存領域へ接しない外周はseam gateから除外する。sourceとinpaint候補のinpaint領域差分はprovenance用に記録するが、正しい隠れ部分をsource一致で拒否しない。
