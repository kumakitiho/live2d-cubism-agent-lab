...
maskはsourceと同じcanvas size・originを保つ。target maskはAA grayscale値を保持し、protect/inpaint maskは用途ごとのalpha thresholdでbinary判定する。対象領域、保護領域、補完余白を分離し、最終import PSDへ生maskを残さない。
